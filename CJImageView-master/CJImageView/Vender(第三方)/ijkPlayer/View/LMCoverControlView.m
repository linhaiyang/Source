...
- (void)syncCoverImageViewWithURLString:(NSString *)urlString placeholderImage:(UIImage *)placeholderImage {
    // 设置网络占位图片
    if (urlString.length) {
        [self.backgroundImageView sd_setImageWithURL:[NSURL URLWithString:urlString] placeholderImage:[UIImage imageNamed:[NSString stringWithFormat:@"IJK.bundle/%@", @"封面占位"]]];;
    } else {
...
- (UIImageView *)playerImageView {
    if (!_playerImageView) {
        ...
        _playerImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"IJK.bundle/%@", @"btn_playplus"]];
